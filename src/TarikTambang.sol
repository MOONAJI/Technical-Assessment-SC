...
contract TarikTambang {
    ...
    {
        return (gameActive, gameFinalized, gameDeadline, totalTeamA, totalTeamB, result);
    }

    function getUserContributions(address user)
        external
        view
        returns (uint256 teamAContribution, uint256 teamBContribution)
    {
        return (teamAContributions[user], teamBContributions[user]);
    }

    function getParticipantCounts() external view returns (uint256 teamACount, uint256 teamBCount) {
        return (teamAParticipants.length, teamBParticipants.length);
    }
}
